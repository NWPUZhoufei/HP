...
class ImageLoader(torch.utils.data.Dataset):
    def __init__(self, root_path='/data/miniImageNet/', transform=None, is_train=False, is_val=False, is_test=False, target_transform=None, select_class=None, k_shot=None, seed=0):
        ...
        if is_train:
            train_folder = root_path + 'miniImageNet_category_split_train.pickle'
            print('load train dataset')
        if is_val:
            train_folder = root_path + 'miniImageNet_category_split_val.pickle'
...
class GeneratorSupportSampler(Sampler):
    def __init__(self, n_class, n_support_pairs, N_reference_per_class):
        self.n_class = n_class
        self.n_support_pairs = n_support_pairs
        self.N_reference_per_class = N_reference_per_class

    def __iter__(self):
        feature_list = range(600)
        class_list = range(self.n_class)
        class_list = np.random.choice(class_list, self.n_support_pairs)
        feature_idx = np.random.choice(feature_list, self.N_reference_per_class, replace=False)
        batch = []
        for j in class_list:
            feature_idx = np.random.choice(feature_list, self.N_reference_per_class, replace=False)
            batch.append([i+j*600 for i in feature_idx])
